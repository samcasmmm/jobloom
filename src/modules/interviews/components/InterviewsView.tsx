    ...
    <div className='w-full space-y-6 text-left'>
      ...
      <div className='flex flex-wrap items-center justify-between gap-4 p-4 rounded-sm bg-[#090912]/80 border border-white/8'>
        ...
        <div className='flex flex-wrap items-center gap-3 text-xs'>
          {/* 2 Option Toggle: Cards / Calendar */}
          <div className='h-8 p-0.5 flex items-center gap-0.5 bg-white/4 rounded-sm border border-white/10'>
            <button
              type='button'
              onClick={() => setViewMode('calendar')}
              className={cn(
                'h-full px-3 rounded-xs font-mono font-medium transition-all cursor-pointer flex items-center gap-1.5 text-xs',
                viewMode === 'calendar'
          ...
          <div className='h-8 p-0.5 flex items-center gap-0.5 bg-white/4 rounded-sm border border-white/10'>
            {(['All', 'Pending', 'Pass', 'Fail'] as const).map((outcome) => (
              <button
                ...
                className={`h-full px-2.5 rounded-xs font-mono font-medium transition-all cursor-pointer flex items-center justify-center text-xs ${
      ...
      {/* Main View Area: Calendar vs Cards */}
      {viewMode === 'calendar' ? (
